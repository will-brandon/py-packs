...
output_ostream = sys.stdout
"""
Determines which output stream program result output will be written to. This includes data that the
has specifically been requested. This is helpful for CLI APIs.
"""
...
interface_ostream = sys.stdout
"""
Determines which output stream user interface messages will be written to.
"""

interface_istream = sys.stdin
"""
Determines which input stream user interface responses will be read from.
"""
...
def output(content: str='', newline: bool=True) -> None:
    ...
    # Write the content and append a newline if the option is true. Flush the stream to ensure the
    # content is written.
    output_ostream.write(f'{content}\n' if newline else content)
...
def confirm(msg: str=None) -> bool:
    ...
    match option:
        ...
        case 'n':
            interface_ostream.write('Aborting.\n')
            interface_ostream.flush()
            return False
        ...
        case _:
            interface_ostream.write('Invalid option, aborting.\n')
            interface_ostream.flush()
            return False
